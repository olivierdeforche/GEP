...
function process_parameters!(m::Model, data::Dict)
    ...
    for country in C
        m.ext[:parameters][:max_import][country] = Dict()
        for k in K
            m.ext[:parameters][:max_import][country][k] = imp[!,country][k]
        end
    end 
    ...
    for c in C        
        m.ext[:parameters][:max_export][country] = Dict()
        for k in K
            m.ext[:parameters][:max_export][country][k] = exp[!,country][k]
        end
    end 
    return m
end
...
function build_greenfield_1Y_GEP_model!(m::Model)
    ...
    imp = m.ext[:variables][:imp] = @variable(m,[c=C,k=K,jh=JD,jd=JD], lower_bound=0, base_name="Import")
    exp = m.ext[:variables][:exp] = @variable(m,[c=C,k=K,jh=JD,jd=JD], upper_bound=0, base_name="Import")
    ...
    g[c,"On-Wind",jh,jd] <= sum(perc["On-wind"][c][z]*AF["On-Wind"][z][jh,jd]*cap_ren_Won[c,z] for z in CWon)
    )

    # 3a2 - Offshore wind 
    m.ext[:constraints][:con3a2] = @constraint(m, [c=C,jh=JH,jd=JD],
    g[c,"Off-Wind",jh,jd] <= sum(perc["Off-wind"][c][z]*AF["Off-Wind"][z][jh,jd]*cap_ren_Wof[c,z] for z in CWof)
    )

    # 3a3 - Onshore solar 
    m.ext[:constraints][:con3a3] = @constraint(m, [c=C,jh=JH,jd=JD],
    g[c,"Solar",jh,jd] <= sum(perc["Solar"][c][z]*AF["Solar"][z][jh,jd]*cap_ren_Son[c,z] for z in CSon)
    )
...
cap_conv = value.(m.ext[:variables][:capc]["Albania",:])
cap_res_won = value.(m.ext[:variables][:cap_r_won]["Albania",:])
cap_res_wof = value.(m.ext[:variables][:cap_r_wof]["Albania",:])
cap_res_son = value.(m.ext[:variables][:cap_r_son]["Albania",:])
g = value.(m.ext[:variables][:g]["Albania",:,:,:])
ens = value.(m.ext[:variables][:ens]["Albania",:,:])
# curt = value.(m.ext[:expressions][:curt])
λ = dual.(m.ext[:constraints][:con2a]["Albania",:,:])
