...
cap_conv_alb = value.(m.ext[:variables][:capc][4,:])
...
g_alb = value.(m.ext[:variables][:g][4,:,:,:])
ens_alb = value.(m.ext[:variables][:ens][4,:,:])
# curt = value.(m.ext[:expressions][:curt])
λ_alb = dual.(m.ext[:constraints][:con2a][4,:,:])

# # create arrays for plotting
λvec_alb = [λ_alb[jh,jd] for jh in JH, jd in JD]
gvec_alb = [g_alb[i,jh,jd] for i in I, jh in JH, jd in JD]
cap_res_wonvec_alb = sum(perc["On-Wind"][4,z]*cap_res_won_alb[z] for z in CWon)
cap_res_wofvec_alb = sum(perc["Off-Wind"][4,z]*cap_res_wof_alb[z] for z in CWof)
cap_res_sonvec_alb = sum(perc["Solar"][4,z]*cap_res_son_alb[z] for z in CSon) 
...
plot!(p2, JH, D[4,:,jd], label ="Demand", xlabel="Timesteps [-]", ylabel="Generation [MWh]", legend=:outertopright, lindewidth=3, lc=:black);# Capacity
